...
import { decryptWIF, generateEncryptedWIF,loginWIF } from '../api/crypto'
...
import {currentuser,getCurrencyCode,getUserRole,getUserData,updateUserRole,updateCurrency,getAllAdress} from '../api/user/login'

let bgTaskHandler = null

export function* rootWalletSaga() {
    yield all([watchCreateWallet(),watchCreateNewWallet(), watchLoginWallet(),watchToggleUser(),watchUpdateCurrency(), watchSendAsset(), watchClaimGAS(), watchImportNEP6()])
}
...
export function* watchToggleUser(){
    while(true){        
        ...
        yield call(updateUserRole,wallet.userId,wallet.roleType)
        yield put({ type: actions.wallet.TOGGLE_USER_SUCCESS })
    }
}
...
function* decryptWalletKeys(encryptedKey, passphrase) {
    try {
     ...
     //   yield call(delay, 1000) // to allow UI to update before it gets locked by the computational heavy decrypt_wif function
        ...
        const userId=yield call(currentuser)
...
export function* retrieveData() {
    let BLOCKCHAIN_UPDATE_INTERVAL = 1000000 //15000
    const wallet = yield select(getWallet)
