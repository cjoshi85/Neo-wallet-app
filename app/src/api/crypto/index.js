...
export function encryptWIF(wif, passphrase) {
    ...
    if (account) {
        ...
        return c.then((result) => {
            var derived = Buffer.from(result.toString())
...
export function decryptWIF(encryptedWIF, passphrase) {
    ...
    //const wif1= wallet.decrypt('6PYSt75tG7udhine9pqSXwsEjck3ApR6Zb2RttXZhcRGEheGNy6bx3zAcu','123456')
    debugger
 ...
 * @returns {string|undefined} If successful the encrypted key in Base58 (Case sensitive) otherwise undefined
 */
export async function generateEncryptedWIF(passphrase, existingWIF) {

    const privatekey=wallet.generatePrivateKey()
    const account = new wallet.Account(existingWIF || wallet.generatePrivateKey())
    const { WIF, address } = account
    const check=await new wallet.Account(WIF)
    debugger
