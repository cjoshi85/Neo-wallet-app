...
// import { ScrollView, View, Text, StyleSheet, TextInput, Clipboard,TouchableOpacity } from 'react-native'
...
class GeneratedKeysView extends React.Component {
    ...
    save = async ()  =>{
        const { passphrase, address, encryptedWIF, name,uid } = this.props

        const response= await Database.listenallwallets(uid)

        alert(response)

        // if (uid && address && passphrase && name  ) {
               
        //   const response= Database.setWalletData(uid,address,passphrase,encryptedWIF,name);
        //   if(response){
        //       alert('Wallet has been saved')
        //   }
        //    }
    ...
    render() {
        const { passphrase, address, encryptedWIF, wif,name } = this.props
        return (
            <ScrollView>
                ...
                <View style={styles.dataList}>
                    <KeyDataRow title="Name" value={name} />
                ...
                <Button
                    ...
                    title="Email data"
                />
                <TouchableOpacity onPress={this.save} activeOpacity={.5}>
